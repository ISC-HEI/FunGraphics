...
object FunGraphics {
	...
	def main(args: Array[String]): Unit = {
		val fg = new FunGraphics(500, 500, "Checking basic capabilities")
		fg.setPixel(10, 10)
		fg.gameloopSample()

		// Testing resources access
		new GraphicsBitmap("/res/img/mandrill.jpg")
	}
...
class FunGraphics(val width: Int, val height: Int, val xoffset: Int, val yoffset: Int, val title: String, val high_quality: Boolean)
	...
	private[graphics] def gameloopSample(): Unit = {
		...
		val yellow_trans: Color = new Color(yellow.getRed, yellow.getRed, yellow.getBlue, 65)
		...
		while (true) {
			if (pressedUp) size += 1
			if (pressedDown) size = if (size == 0) 0 else size - 1
			frontBuffer.synchronized {
				clear(Color.white)
				setColor(Color.red)
				drawFilledOval(50, 50, 100 + size, 100 + size)
				drawString(50, 250, "FunGraphics " + FunGraphics.version)
				setColor(yellow_trans)
